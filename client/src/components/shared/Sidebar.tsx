...
import { Home, Building2, User, HelpCircle, LogOut, PanelRight } from 'lucide-react'
...
const Sidebar: React.FC<SidebarProps> = ({ open, setOpen }) => {
    ...
    const menuItems = [
        ...
        { icon: Building2, name: 'Companies', path: '/companies' },
        ...
        >
            <div className="flex items-center justify-between border-b p-4">
                <div className="flex items-center space-x-2">
                    <img src='https://res.cloudinary.com/dkawvablj/image/upload/v1735713425/ProFollow/s7fbw6d8fmb29mfocfe6.webp' alt="Logo" className='h-16 w-16' />
                    <div className="flex flex-col">
                        <span className="text-xl font-semibold">Pro Follow</span>
                        <span className="text-xs font-thin text-gray-600">Admin Dashboard</span>
                    </div>
                </div>
            ...
            <nav className="flex-grow mt-5">
                {menuItems.map((item) => (
                    <Link
                        key={item.name}
                        to={item.path}
                        className={`flex items-center px-6 py-3 text-gray-600 hover:bg-gray-100 ${location.pathname === item.path ? 'bg-gray-100' : ''
                            }`}
                    >

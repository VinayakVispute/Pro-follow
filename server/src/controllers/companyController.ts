...
export const createCompany = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        if (!name || !emails || emails.length === 0) {
            console.log("Validation failed: Name and at least one email are required.");
            res.status(400).json({
                success: false,
...
export const updateCompany = async (req: Request, res: Response): Promise<void> => {
    console.log("updateCompany called with params:", req.params, "and body:", req.body);
    try {
        const { id } = req.params;
        ...
        const updatedCompany = await Company.findByIdAndUpdate(id, updatedCompanyBody, { new: true, });
